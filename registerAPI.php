<?php   
  ...
  case 'signup':  
    ...
    if($stmt->num_rows > 0){  
        ...
        if($stmt->execute()){  
            $stmt = $conn->prepare("SELECT user_id, user_id, username, email, gender FROM user WHERE username = ?");   
...
case 'login':  
  if(isTheseParametersAvailable(array('username', 'password'))){  
    ...
    $stmt = $conn->prepare("SELECT user_id, username, email, gender FROM user WHERE username = ? AND password = ?");  
    ...
    $response['error'] = false;   
    $response['message'] = 'Invalid username '. $username. ' or password '.$password;  
 }  
}  
break;   
default:   
